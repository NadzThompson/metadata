...
#
# retriever_enhanced.py
#
# ENHANCED VERSION — Original Retriever class preserved with NOVA Three-Rule metadata added.
#
# Enhancements:
...
# Try importing docx processor (optional dependency)
try:
...
@dataclass
class NOVARetrievalFilters:
    """Structured NOVA metadata filters for retrieval queries (Rule 2)."""
    regulator: Optional[str] = None
    jurisdiction: Optional[str] = None
    authority_class: Optional[str] = None
    nova_tier: Optional[str] = None
    document_class: Optional[str] = None
    source_type: Optional[str] = None  # 'regulatory' or 'internal'
    status: Optional[str] = None
    business_line: Optional[str] = None
    confidentiality: Optional[str] = None
    sector: Optional[str] = None
    normative_weight: Optional[str] = None
    paragraph_role: Optional[str] = None
    structural_level: Optional[str] = None
...
# ============================================================================
# ORIGINAL RETRIEVER CLASS — ENHANCED WITH NOVA
# ============================================================================

class Retriever:
    """
    Manages file retrieval, indexing, and searching across multiple vector store backends.
    ENHANCED with NOVA Three-Rule metadata architecture.
    """
    ...
    # ==========================================================================
    # NOVA-ENHANCED SEARCH METHODS
    # ==========================================================================

    def search_with_nova_filters(self, query_text, nova_filters=None, top_k=5, auto_detect=True):
        """
        Search with NOVA metadata filters (Rule 2) and return results with
        prompt-injected metadata (Rule 3).

        Args:
            query_text: The search query
            nova_filters: NOVARetrievalFilters instance or dict of filter fields
            top_k: Number of results to return
            auto_detect: If True, auto-detect filters from query text

        Returns:
            List of results with NOVA metadata context for prompt injection
        """
        if not self.search_client:
            raise ValueError("Search client not initialized")
        ...
        filter_dict = {}
        if auto_detect:
            filter_dict.update(auto_detect_filters_from_query(query_text))

        if nova_filters:
            if isinstance(nova_filters, NOVARetrievalFilters):
                filter_dict.update(nova_filters.to_filter_dict())
            elif isinstance(nova_filters, dict):
                filter_dict.update(nova_filters)

        # Build OData filter expression for Azure AI Search
        filter_clauses = []
        for field, value in filter_dict.items():
            if isinstance(value, bool):
                filter_clauses.append(f"{field} eq {str(value).lower()}")
        ...
        try:
            # Perform search with NOVA filters
            search_kwargs = {
            ...
            # Format results with NOVA prompt injection (Rule 3)
            nova_results = []
            for result in results:
                metadata = json.loads(result.get("metadata", "{}"))
                chunk_text = result.get("chunk", result.get("content", ""))

                # Apply Rule 3: inject metadata for LLM prompt
                prompt_text = render_chunk_for_prompt(metadata, chunk_text)

                nova_results.append({
                    "id": result.get("id"),
                    "content": chunk_text,
                    "prompt_text": prompt_text,  # Rule 3 output
                    "metadata": metadata,
                    "score": result.get("@search.score", 0),
                    "source_type": metadata.get("source_type", ""),
                    "regulator": metadata.get("regulator", ""),
                    "normative_weight": metadata.get("normative_weight", ""),
                    "filters_applied": filter_dict,
                })

            logger.info(f"NOVA search: {len(nova_results)} results "
                       f"(filters: {filter_dict})")
            return nova_results

        except Exception as e:
            logger.error(f"Error in NOVA search: {e}")
            raise

    def retrieve_with_nova_context(self, query_text, nova_filters=None, top_k=5):
        """
        Retrieve documents with full NOVA metadata context.
        Returns results ready for prompt assembly with Rule 3 metadata headers.

        Args:
            query_text: Search query
            nova_filters: Optional NOVA filters
            top_k: Number of results

        Returns:
            dict with 'results' list and 'prompt_context' (assembled prompt text)
        """
        results = self.search_with_nova_filters(query_text, nova_filters, top_k)

        # Assemble full prompt context from all results
        prompt_sections = []
        for i, result in enumerate(results, 1):
            prompt_sections.append(f"[Source {i}]\n{result['prompt_text']}\n")

        return {
            "results": results,
            "prompt_context": "\n".join(prompt_sections),
            "num_results": len(results),
            "filters_applied": results[0].get("filters_applied", {}) if results else {},
        }

    def search_regulatory(self, query_text, regulator=None, jurisdiction=None, top_k=5):
        """
        Convenience method for searching regulatory documents.
        Auto-sets source_type='regulatory' and optional regulator/jurisdiction filters.
        """
        filters = NOVARetrievalFilters(
            source_type="regulatory",
            regulator=regulator,
            jurisdiction=jurisdiction,
            status="active",
        )
        return self.search_with_nova_filters(query_text, filters, top_k)

    def search_internal(self, query_text, business_line=None, business_owner=None, top_k=5):
        """
        Convenience method for searching internal policy documents.
        Auto-sets source_type='internal' and optional business context filters.
        """
        filters = NOVARetrievalFilters(
    ...
    def search_definitions(self, query_text, regulator=None, top_k=5):
        """Search specifically for definition chunks."""
        filters = NOVARetrievalFilters(
    ...
    def search_requirements(self, query_text, regulator=None, top_k=5):
        """Search specifically for mandatory requirement chunks."""
        filters = NOVARetrievalFilters(
    ...
    # ==========================================================================
    # TEMPORAL VERSIONING — As-of-Date Query
    # ==========================================================================

    def search_as_of_date(self, query_text, as_of_date, regulator=None, top_k=5):
        """Search for documents that were effective on a specific date.

        Critical for temporal regulatory queries like:
          "What were the LCR requirements as of June 2024?"

        Both current and superseded versions remain in the index.
        This method filters to only those whose effective_date window
        covers the specified date.

        Args:
            query_text: search query
            as_of_date: ISO date string (e.g., '2024-06-15')
            regulator: optional regulator filter
            top_k: number of results

        Returns:
            List of results with NOVA metadata, filtered to the temporal window
        """
        if not self.search_client:
            raise ValueError("Search client not initialized")
        ...
        #   AND (effective_date_end >= as_of_date OR effective_date_end is null)
        filter_parts = [
            f"effective_date_start le '{as_of_date}'",
            f"(effective_date_end ge '{as_of_date}' or effective_date_end eq null)",
        ]
        if regulator:
            filter_parts.append(f"regulator eq '{regulator}'")
        ...
        try:
            ...
            for result in results:
                ...
                nova_results.append({
                    ...
                    "effective_date_start": metadata.get("effective_date_start", ""),
                    "effective_date_end": metadata.get("effective_date_end", ""),
                })

            logger.info(f"Temporal search as-of {as_of_date}: {len(nova_results)} results")
            return nova_results

        except Exception as e:
            logger.error(f"Error in as-of-date search: {e}")
            raise

    # ==========================================================================
    # PGVector RETRIEVAL — Dense Cosine Similarity
    # ==========================================================================

    def search_pgvector(self, query_text, nova_filters=None, top_k=5, pg_table="nova_chunks"):
        """Search PGVector for dense cosine similarity matches.

        This is the second retrieval path (alongside Azure AI Search / ES).
        Results can be merged with ES results via Reciprocal Rank Fusion
        using retrieve_dual_store().

        Args:
            query_text: search query
            nova_filters: NOVARetrievalFilters or dict
            top_k: number of results
            pg_table: PGVector table name

        Returns:
            List of result dicts with NOVA metadata
        """
        try:
            import psycopg2
            from psycopg2.extras import RealDictCursor
        except ImportError:
            logger.warning("psycopg2 not installed — PGVector search unavailable")
            return []
        ...
        if nova_filters:
            if isinstance(nova_filters, NOVARetrievalFilters):
                filter_dict = nova_filters.to_filter_dict()
            elif isinstance(nova_filters, dict):
                filter_dict = nova_filters
        ...
        try:
            pg_config = self.config.get("pgvector", {})
            conn = psycopg2.connect(
                host=pg_config.get("host"),
                port=pg_config.get("port", 5432),
                dbname=pg_config.get("database", "nova"),
                user=pg_config.get("user"),
                password=pg_config.get("password"),
            )

            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute(query_sql, [str(query_embedding)] + params + [str(query_embedding), top_k])
                rows = cur.fetchall()
            ...
            for row in rows:
                doc = dict(row)
                score = doc.pop("cosine_similarity", 0.0)
                doc.pop("embedding", None)  # remove raw vector

                prompt_text = render_chunk_for_prompt(doc, doc.get("content", ""))
    ...
    def retrieve_dual_store(self, query_text, nova_filters=None, top_k=5,
                            use_search=True, use_pgvector=True, rrf_k=60):
        """Retrieve from both Azure AI Search AND PGVector, merge via Reciprocal Rank Fusion.

        This is the recommended retrieval method for production — it combines
        BM25 + dense from Azure AI Search with pure dense from PGVector for
        maximum recall.

        Args:
            query_text: search query
            nova_filters: NOVA filters
            top_k: final number of results after merging
            use_search: whether to query Azure AI Search
            use_pgvector: whether to query PGVector
            rrf_k: RRF constant (default 60, standard value)

        Returns:
            dict with merged results, prompt_context, and per-source hit counts
        """
        all_results = []
        ...
        if use_search:
            try:
                search_results = self.search_with_nova_filters(
                    query_text, nova_filters, top_k=top_k * 2
                )
                all_results.append(("search", search_results))
            except Exception as e:
                logger.warning(f"Azure AI Search retrieval failed: {e}")

        # Retrieve from PGVector
        pg_results = []
        if use_pgvector:
            try:
                pg_results = self.search_pgvector(
                    query_text, nova_filters, top_k=top_k * 2
                )
                all_results.append(("pgvector", pg_results))
            except Exception as e:
        ...
        for source_name, results in all_results:
            for rank, result in enumerate(results, 1):
                chunk_id = result.get("id", str(rank))
        ...
        for i, result in enumerate(merged, 1):
            prompt_sections.append(f"[Source {i}]\n{result.get('prompt_text', result.get('content', ''))}\n")
        ...
        return {
            ...
            "pgvector_hits": len(pg_results),
            "retrieval_method": "dual_store_rrf",
        }

    # ==========================================================================
    # END NOVA ENHANCEMENTS
    # ==========================================================================
